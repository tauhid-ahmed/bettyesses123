...
export function DashboardHeaderContainer({ children }: DashboardSidebarProps) {
  return (
    <div className="[grid-area:header] bg-white border-b border-gray-100 shadow-[0_1px_2px_rgba(0,0,0,0.1)]">
      {children}
    </div>
  );
}
...
export function DashboardMainContainer({ children }: DashboardMainProps) {
  return (
    <div className="[grid-area:main] overflow-y-auto overflow-x-auto">
      {children}
    </div>
  );
}
